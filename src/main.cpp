...
int main(int argc, char *argv[])/*{{{*/
{
    ...
    try {
        if (argc > 4 || argc < 2)
            throw Control::Control::ReturnParametersInvalid;
        ...
        for (int i = 1; argv[i]; ++i) {/*{{{*/
            if (std::string(argv[i]) == "-p") {
                if (port >= -1) // The port can only be specified once.
                    throw Control::Control::ReturnParametersInvalid;
                if (!argv[++i])
                    throw Control::Control::ReturnPortUnspecified;
                ...
                || port > 65535)
                    throw Control::Control::ReturnPortInvalid;
            } else if (argv[i][0] == '-')
                throw Control::Control::ReturnParametersInvalid;
            // The JID can only be specified once.
            else if (jid.username().empty()) {
                if (!jid.setJID(argv[i]) || jid.username().empty())
                    throw Control::Control::ReturnJidInvalid;
            } else
                throw Control::Control::ReturnParametersInvalid;
        }/*}}}*/

        if (port == -2) port = -1; // Port was not set by parameter.
            throw Control::Control::initialize(jid, port);
    } catch (const Control::Control::ReturnCode returnCode) {
        return Control::Control::printError(returnCode, argv[0]);
    }
}/*}}}*/
